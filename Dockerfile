...
CMD ["bash", "-c", "echo '4DGS RunPod Container Ready!' && echo '' && echo 'Environment: Ubuntu 22.04 with PyTorch 2.8.0 + CUDA 12.8' && echo '' && echo 'Available tools:' && echo '  - preprocess_video.py: Process a folder of videos' && echo '  - train.py: Train 4DGS models' && echo '  - render.py: Render trained models' && echo '  - export_ply.py: Export to PLY format' && echo '' && echo 'Example usage:' && echo '  python3 tools/preprocess_video.py /videos -o /data' && echo '  python3 tools/train.py --data_root /data --out_dir /model' && echo ''"]
