...
def main():
    ...
    # Compute total images from mapping groups (per-frame-per-camera)
    total_images = sum(len(g.get('images', {})) for g in mapping.get('groups', []))
    print(f"Images (from mapping): {total_images}")
